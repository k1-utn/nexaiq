...
Stage 6 is underway with a Windows tray application, authenticated workstation registration, current-user encrypted session storage, folder scanning, an on-disk retry queue, idempotent private EMS file sync, version tracking, audit history, and tenant-isolation tests. EMS parsing and repair-order matching remain deliberately disabled until an authorized real export sample is inspected. See [the Stage 6 architecture note](docs/architecture/STAGE_6_WINDOWS_EMS_CONNECTOR.md).
